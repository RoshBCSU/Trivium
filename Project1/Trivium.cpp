...
class Trivium
{
    ...
    // Function to check if a decimal string is greater than the max value
    ...
    string binaryAddition(string& binary1, string& binary2) {
        // Ensure both binary strings have the same length (80 bits)
        ...
        // Perform binary addition from right to left (least significant bit to most significant)
        for (int i = 79; i >= 0; i--) {
            ...
            result.insert(0, 1, '0' + (sum % 2)); // Add the least significant bit to the front
        }

        // If there's a carry left (overflow), add it as the most significant bit
        if (carry > 0) {
            result.insert(0, 1, '1');
        }
    ...
    string ConvertToBin(string Val){
        ...
        while (isGreaterThanMaxValue(Val))
        {
            ...
            Val = Remainder; 
            // Store the current value as an intermediate result
    ...
    string generateKeyStream(unsigned long long plainTxtLen){
        ...
        for(int i = 0; i < plainTxtLen; i++){
            ...
            // cout << "\nt1: " << t1 << " s66: " << (int)RegA.getBitAt(27) << " s93: " << (int)RegA.getBitAt(0) << endl;
            // cout << "t2: " << t2 << " s162: " << (int)RegB.getBitAt(15) << " s177: " << (int)RegB.getBitAt(0) << endl;
            // cout << "t3: " << t3 << " s243: " << (int)RegC.getBitAt(45) << " s288: " << (int)RegC.getBitAt(0) << endl;
            // cout << "currOutput: " << currOutput << endl; 
            // cout << "keySteram: " << keyStream << endl;
            // cout << "\n========================\n";

            //RegA --> 2 = s91, 1 = s92 RegB --> 6 = s171
            newT1 = t1 ^ ((int)RegA.getBitAt(2)&(int)RegA.getBitAt(1)) ^ (int)RegB.getBitAt(6);

            //RegB --> 2 = s175, 1 = s176 RegC --> 24 = s264
            newT2 = t2 ^ ((int)RegB.getBitAt(2)&(int)RegB.getBitAt(1)) ^ (int)RegC.getBitAt(24);

            //RegC --> 2 = s286, 1 = s287 RegA --> 24 = s69
            newT3 = t3 ^ ((int)RegC.getBitAt(2)&(int)RegC.getBitAt(1)) ^ (int)RegA.getBitAt(24);

            // cout << "\nnewT1: " << newT1 << " t1: " << t1 << " s91: " << (int)RegA.getBitAt(2) << " s92: " << (int)RegA.getBitAt(1) << " s171: " << (int)RegB.getBitAt(6) << endl;
            // cout << "\nnewT2: " << newT2 << " t2: " << t2 << " s175: " << (int)RegB.getBitAt(2) << " s176: " << (int)RegB.getBitAt(1) << " s264: " << (int)RegC.getBitAt(24) << endl;
            // cout << "\nnewT3: " << newT3 << " t3: " << t3 << " s286: " << (int)RegC.getBitAt(2) << " s287: " << (int)RegC.getBitAt(1) << " s69: " << (int)RegA.getBitAt(24) << endl;
            // cout << "\n========================\n";
